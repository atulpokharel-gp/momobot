...
function TaskTypeSelector({ selected, onChange, customTaskTypes = [] }) {
    ...
    <div className="relative">
      ...
      >
        <span className="flex items-center gap-2">
          {selected && (
            <span>
              {customTaskTypes.find(ct => ct.type === selected) 
                ? customTaskTypes.find(ct => ct.type === selected).icon
                : AGENT_CAPABILITIES[selected]?.icon}
            </span>
          )}
          {selected ? (
            customTaskTypes.find(ct => ct.type === selected)?.name || AGENT_CAPABILITIES[selected]?.name || 'Unknown'
          ) : (
            'Select a task type'
          )}
        </span>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-gray-700 rounded-lg shadow-lg z-20 max-h-96 overflow-y-auto">
          {/* Standard Task Types */}
          <div>
            <div className="sticky top-0 px-4 py-2 bg-gray-800 border-b border-gray-700 text-xs font-semibold text-gray-400 uppercase">
              Built-in Tasks
            </div>
          ...
          {customTaskTypes.length > 0 && (
            <div>
              <div className="sticky top-0 px-4 py-2 bg-gray-800 border-t border-b border-gray-700 text-xs font-semibold text-brand-400 uppercase">
                ⭐ Custom Workflows
              </div>
              {customTaskTypes.map((customTask) => (
                <button
                  key={customTask.type}
                  onClick={() => {
                    onChange(customTask.type)
                    setOpen(false)
                  }}
                  className="w-full px-4 py-3 text-left hover:bg-gray-800 border-b border-gray-800 last:border-b-0 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <span className="text-xl">{customTask.icon}</span>
                    <div>
                      <p className="font-medium text-white">{customTask.name}</p>
                      <p className="text-xs text-gray-500">{customTask.description || 'Custom workflow task'}</p>
                    </div>
                  </div>
                </button>
              ))}
...
export default function TaskCreationPage() {
  ...
  // Load agents and custom task types
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (agentsRes.data.agents?.length > 0) {
          setSelectedAgent(agentsRes.data.agents[0].id)
        }
        setCustomTaskTypes(taskTypesRes.data.taskTypes || [])
      } catch (error) {
  ...
  const generateWorkflow = (thinking = null) => {
    ...
    // Check if it's a custom task type
    const customTask = customTaskTypes.find(ct => ct.type === taskType)
    if (customTask) {
      // Use the saved workflow definition for custom tasks
      const savedWorkflow = JSON.parse(customTask.workflow_definition)
  ...
  const executeTask = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        type: taskType,
        description: description || (customTask?.description || AGENT_CAPABILITIES[taskType]?.name),
        parameters: params,
        ai_model: aiModel,
      ...
      if (res.data.success) {
        ...
        if (scheduleTask && taskId) {
          try {
            ...
            const customTask = customTaskTypes.find(ct => ct.type === taskType)
            const taskName = customTask?.name || AGENT_CAPABILITIES[taskType]?.name
            
            const scheduleRes = await api.post('/schedules', {
              name: scheduleName || `${taskName} - ${scheduleFrequency}`,
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* Task Type */}
              <div>
                <TaskTypeSelector selected={taskType} onChange={setTaskType} customTaskTypes={customTaskTypes} />
              </div>
            </div>
